...
const AvatarPreview = ({ name }) => {
  const initials = name
    ? name.split(' ').filter(Boolean).map(w => w[0]).slice(0, 2).join('').toUpperCase()
    : '';
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px', border: '1px solid var(--border-color)', borderRadius: '12px', backgroundColor: 'var(--surface-color)', marginTop: 8 }}>
       <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'linear-gradient(135deg, var(--primary-color), var(--primary-hover))', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 20, fontWeight: 700, flexShrink: 0 }}>
          {initials || <UserIcon />}
       </div>
       <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>{name || 'Your Name'}</span>
          <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>Profile avatar will be generated from your initials</span>
       </div>
    </div>
  );
};
      ...
      <div style={{ backgroundColor: 'var(--surface-color)', padding: '48px 40px', borderRadius: 24, width: '100%', maxWidth: 640, boxShadow: '0 4px 24px rgba(0,0,0,0.02)' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <input type="tel" placeholder="+91 98765 43210" required value={formData.phoneNumber} onChange={e => updateForm('phoneNumber', e.target.value)} style={inputStyle} disabled={isLoading} />
              </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 16 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                  <input type={showPassword ? "text" : "password"} placeholder="Create a password" required value={formData.password} onChange={e => updateForm('password', e.target.value)} style={{...inputStyle, paddingRight: 40, width: '100%'}} disabled={isLoading} />
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                  <input type={showConfirmPassword ? "text" : "password"} placeholder="Re-enter your password" required value={formData.confirmPassword} onChange={e => updateForm('confirmPassword', e.target.value)} style={{...inputStyle, paddingRight: 40, width: '100%', borderColor: formData.confirmPassword ? (formData.password === formData.confirmPassword ? 'var(--primary-color)' : 'var(--alert-color)') : 'var(--border-color)'}} disabled={isLoading} />
                ...
                {formData.confirmPassword && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 600, color: formData.password === formData.confirmPassword ? 'var(--primary-color)' : 'var(--alert-color)', marginTop: 2 }}>
                    {formData.password === formData.confirmPassword ? (
                      <><CheckIcon /> Passwords match</>
                    ) : (
                      <><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg> Passwords do not match</>
                    )}
                  </div>
                )}
              </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20, marginTop: 8 }}>
            ...
            <AvatarPreview name={formData.name} />
          </div>
